...
/// Since the block size is 16 and we need to encrypt 32 bytes (to get a 256
...
#[inline]
fn hash_block(block: &[u8; BLOCK_SIZE], hash: &mut [u8; 32]) {
    ...
    for i in 0..16 {
        ...
        // the last XOR operation is used since we call AES two times per block
...
impl AesMmo {
    ...
    pub fn update(&mut self, data: &[u8]) {
        ...
        loop {
            // get the number of bytes available to try to fill a block
            let data_to_copy = usize::min(data.len() - data_offset, BLOCK_SIZE - self.block_offset);

            // update the block data and offset
            ...
            // stop there if there is not enough data to fill a block
            if self.block_offset < BLOCK_SIZE {
                return;
            }

            // hash block and update offsets
    ...
    pub fn finalize(mut self) -> [u8; BLOCK_SIZE] {
        // hash remaining data by padding with 0s
    ...
    #[must_use]
    pub fn digest(seed: &[u8; BLOCK_SIZE], data: &[u8]) -> [u8; BLOCK_SIZE] {
    ...
    /// It will handle hasher creation, data feeding and finalization.
    #[must_use]
    pub fn digest2(seed: &[u8; BLOCK_SIZE], data: &[u8]) -> ([u8; BLOCK_SIZE], [u8; BLOCK_SIZE]) {
        let first_hash = Self::digest(seed, data);
