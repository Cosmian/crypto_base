...
use crate::{
    abe::{decrypt, encrypt, generate_symmetric_key_and_header, generate_user_decryption_key},
    entropy::new_uid,
    hybrid_crypto::Block,
...
fn single_test() -> anyhow::Result<()> {
    ...
    let user_decryption_key = hex::decode(uk)?;
...
#[test]
fn abe_multiple_encrypt_single_decrypt() -> anyhow::Result<()> {
    let policy = Policy::new(10)
        .add_axis("Departments", &["FR", "AU", "DE"], false)?
        .add_axis("Levels", &["Sec_level_1", "Sec_level_2"], true)?;
