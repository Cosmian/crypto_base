...
use crate::{
    asymmetric::{
        abe::{AbeCrypto, AbePrivateKey, AbePublicKey},
        AsymmetricCrypto,
    },
...
pub fn generate_delegation_key(
    ...
    let user_decryption_key =
        GpswDecryptionKey::<Bls12_381>::from_bytes(&hex::decode(user_decryption_key)?)?;
...
pub fn revoke_attributes(
    ...
    policy: &Policy,
) -> anyhow::Result<Engine<Gpsw<Bls12_381>>> {
    //
    // Build engine from configuration file
    //
...
pub fn generate_symmetric_key_and_header(
    ...
    policy: &Policy,
) -> anyhow::Result<([u8; KEY_LENGTH], Vec<u8>)> {
    //
    // Build engine from configuration file
    //
    ...
    let header = asymmetric_header.to_bytes(symmetric_key.1)?;
...
pub fn encrypt(
    ...
    let symmetric_key = Key::try_from(symmetric_key)?;
    ...
    for block_number in 0..nb_blocks {
        ...
        block.write(0, &input[start..end])?;
        let cipher_text = block.to_encrypted_bytes(
            &symmetric,
...
fn build_abe(encrypted_data: &[u8]) -> anyhow::Result<Abe> {
    trace!("Encrypted data: {:?}", &encrypted_data);
    anyhow::ensure!(
        encrypted_data.len() > ATTRIBUTES_FIELD_SIZE,
        "Encrypted data size cannot be less than {} bytes",
    ...
    anyhow::ensure!(
        nb_attributes < u32::from(u16::MAX),
        "Could not decode number of attributes in encrypted message"
    );
...
pub fn get_header_and_data(encrypted_data: &[u8]) -> anyhow::Result<(Vec<u8>, Vec<u8>)> {
...
pub fn symmetric_decryption(
    ...
    let sk = Key::try_from(symmetric_key)?;
    //
